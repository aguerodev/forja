...
- **Check del contrato rojo** (`CMD_CHECK`) → NO es un fallo terminal: es un loop de arreglo. Arreglás el gate que falla y re-corrés hasta verde. JAMÁS declares el bootstrap exitoso con el check rojo. Solo aplica cuando el proyecto ya tiene código que defina ese comando (ver Paso 4).
...
- `git`, `node` → **duros**: sin eso no hay proyecto (node es el runtime del tooling forja, no una opinión sobre el stack). El toolchain del stack (pnpm, go, etc.) se verifica con los comandos del contrato en el Paso 4.
...
| DB_USER | slug | `app` |
| DB_NAME | slug | = APP |
| CMD_INSTALL | comando shell | `pnpm install` |
| CMD_CHECK | comando shell | `pnpm run check` |
| CMD_TEST | comando shell | `pnpm test:unit` |
| CMD_VERSION | comando shell (imprime la versión en stdout) | `node -p "require('./package.json').version"` — pasá el comando TAL CUAL, con sus comillas: el instanciador lo escapa al escribir `.forja.json` |
| APP_PORT | entero (puerto interno del contenedor) | 8000 |
| HEALTH_PATH | path que empieza con `/` | `/api/health` |
...
1. `git init -b main`
2. **Solo si el proyecto ya tiene código** que defina los comandos del contrato (manifest de dependencias, código fuente — p. ej. init sobre un repo existente): corré el comando `install` del contrato (`CMD_INSTALL`) y después el `check` (`CMD_CHECK`) — **tiene que salir verde**; si sale rojo, leé el gate que falló, arreglalo y re-corré hasta verde. En un **esqueleto fresco** (el template no trae código de app) saltá este sub-paso y decilo explícitamente: el gate de check aplica desde que exista el código de la app — NO bloquees el cierre de init por eso.
...
5. Primer deploy: **`/forja:deploy preview`** — requiere que el proyecto ya tenga su propio `Dockerfile` cumpliendo el contrato de imagen (targets `runner`/`migrator`/`backup`; doctrina `forja:doctrina`, wiki ops/02). El plugin no trae uno.
