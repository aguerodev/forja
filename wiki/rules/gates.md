...
- Corré el comando `check` del contrato (`commands.check` de `.forja.json`; default `pnpm run check`) y dejalo verde antes de abrir o mergear un PR: local = CI — mismos gates, mismas versiones, mismo veredicto.
...
- Tratá el mutation testing como métrica nightly, no como gate de merge: subirlo a gate es un punto del dial, no el default.
- Si un gate falla, corregí el código o el diseño, nunca el gate: los guardarraíles que importan son ejecutables.

Doctrina: wiki/proceso/02_explicacion-tdd.md, wiki/rules/contrato-forja.md
