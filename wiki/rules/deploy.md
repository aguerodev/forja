...
- Desplegá a producción SOLO con el comando `/deploy`, desde `main` limpio, al día con `origin/main` y con `pnpm run check` verde — NUNCA por CI ni corriendo `deploy.sh` a mano.
...
- Revertí con `/rollback` distinguiendo los dos planos, sin mezclarlos jamás: software (barato, re-apunta la imagen, se ofrece de inmediato) y datos (`pg_restore` del dump pre-migración, destructivo y human-confirmed — borra todo lo escrito después del deploy).
