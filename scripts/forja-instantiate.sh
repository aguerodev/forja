...
#
# Required keys:
#   APP PUBLIC_NAME DOMAIN GH_ORG GH_REPO NODE_VERSION PG_MAJOR DB_USER DB_NAME PROJECT_NAME
# Optional keys:
...
usage() {
  ...
  printf 'Required keys: APP PUBLIC_NAME DOMAIN GH_ORG GH_REPO NODE_VERSION PG_MAJOR DB_USER DB_NAME PROJECT_NAME\n' >&2
  exit 2
}
...
NODE_VERSION="" PG_MAJOR="" DB_USER="" DB_NAME="" PROJECT_NAME=""
...
for kv in "$@"; do
  ...
  value="${kv#*=}"
  case "${key}" in
    APP|PUBLIC_NAME|DOMAIN|GH_ORG|GH_REPO|NODE_VERSION|PG_MAJOR|DB_USER|DB_NAME|PROJECT_NAME|FORJA_MARKETPLACE_REPO)
      printf -v "${key}" '%s' "${value}" ;;
    *) die "unknown key: ${key}" ;;
  esac
done

for k in APP PUBLIC_NAME DOMAIN GH_ORG GH_REPO NODE_VERSION PG_MAJOR DB_USER DB_NAME PROJECT_NAME; do
...
TOKEN_KEYS="APP PUBLIC_NAME DOMAIN GH_ORG GH_REPO NODE_VERSION PG_MAJOR DB_USER DB_NAME PROJECT_NAME FORJA_MARKETPLACE_REPO"
